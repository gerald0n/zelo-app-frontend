-- Cupons: novo tipo `full_order` — desconto de 100% do valor total do
-- pedido (subtotal de produtos + adicionais + entrega, zerando o total).
-- `discount_value` é ignorado, igual a `free_shipping`.

alter table public.coupons
  drop constraint coupons_discount_type_check,
  add constraint coupons_discount_type_check
    check (discount_type in ('percent', 'fixed', 'free_shipping', 'full_order'));

-- `p_add_ons_total_cents` só importa pro tipo `full_order` — os demais
-- tipos continuam incidindo só sobre subtotal de produtos (ou frete), como
-- já documentado no doc 104. Precisa dropar antes: a assinatura muda com o
-- novo parâmetro.
drop function private.coupon_discount_cents(text, integer, integer, integer);

create function private.coupon_discount_cents(
  p_type text,
  p_value integer,
...
as $$
  select case p_type
    ...
    when 'full_order' then
      p_subtotal_cents + p_add_ons_total_cents + p_delivery_fee_cents
    else 0
  end;
$$;

-- Idem: assinatura ganhou `p_add_ons_total_cents` (default 0, então
-- `create_order`/`create_manual_order` continuam válidos até serem
-- atualizados logo abaixo, e `preview_coupon` nem precisa mudar).
drop function private.claim_coupon(text, integer, integer, boolean);
...
-- `create_order`/`create_manual_order`: passam o total de adicionais pro
-- `claim_coupon` poder zerar o pedido inteiro em `full_order`. Resto do
-- corpo idêntico ao de `20260908140000_coupons.sql`.
