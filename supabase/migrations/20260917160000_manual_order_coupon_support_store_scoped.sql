-- White label — Fase C do ADR-0001: reconcilia duas mudanças em
-- `private.create_manual_order` que avançaram em paralelo por branches
-- diferentes e teriam colidido no mesmo `create or replace function`:
--
-- 1. `develop` (PR #142, `20260917150000_manual_order_coupon_support.sql`):
--    reintroduz o suporte a cupom que tinha sido perdido silenciosamente em
--    `20260915140000_manual_order_allow_pix_manual.sql` — mas o
--    `claim_coupon` que essa migration chama é o de 5 parâmetros (sem
--    `p_store_id`), porque `develop` não tem as migrations de tenant desta
--    branch.
-- 2. Esta branch (`20260917150000_tenant_customer_identity_decouple.sql`):
--    resolve `v_store_id` via `private.current_admin_store_id()`, escopa a
--    busca de cliente por telefone por loja, e grava `orders.store_id` — mas
--    não tinha suporte a cupom (regressão já existente antes desta branch
--    começar).
--
-- Esta migration é a junção das duas: todo o corpo de cupom do PR #142,
-- com a chamada a `claim_coupon` usando a assinatura de 6 parâmetros
-- (`p_store_id` incluído, exigida desde `20260917140000_tenant_coupons_
-- unique_and_scoped.sql`), mais a resolução/gravação de `store_id` e a
-- busca de cliente por telefone escopada por loja desta branch.
