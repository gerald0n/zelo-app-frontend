-- Regressão: `private.create_manual_order` perdeu o suporte a cupom em
-- 20260915140000_manual_order_allow_pix_manual.sql, que reescreveu a função
-- inteira pra aceitar 'pix_manual' usando como base uma versão anterior à
-- adição de cupom em 20260912120000_coupon_full_order_discount.sql — não
-- foi decisão deliberada (sem issue, sem menção em ADR/commit), o frontend
-- da comanda manual (apps/admin) continuou mandando `coupon_code` e
-- mostrando o desconto na tela o tempo todo.
--
-- Corpo idêntico ao anterior (20260915140000), só reintroduzindo o mesmo
-- bloco de cupom que `private.create_order` já tem: variáveis
-- v_has_promo/v_coupon_code/v_coupon_id/v_coupon_discount, leitura de
-- `payload->>'coupon_code'`, chamada a `private.claim_coupon` antes de
-- calcular `v_total`, e gravação de coupon_id/coupon_code/coupon_discount_cents
-- no insert de `orders`.
