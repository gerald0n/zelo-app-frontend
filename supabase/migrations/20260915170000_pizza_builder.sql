-- Construtor de pizza (tamanho + até 2 sabores + adicionais por metade)
--
-- Modelo: cada sabor de pizza é um product normal (product_type =
-- 'pizza_flavor'), reaproveitando nome/descrição/imagem/admin/cache/auditoria
-- já existentes. O preço não vem de products.price_cents (ignorado para esse
-- tipo) e sim de pizza_flavor_prices, por tamanho. pizza_sizes nasce com
-- P/M/G mas só G ativo — a UI do cliente pula a etapa de tamanho enquanto
-- houver só 1 tamanho ativo, sem exigir migração nova para ligar P/M depois.
--
-- Adicionais de pizza têm preço diferente se aplicados a "metade" ou à
-- "pizza toda" (pizza_addons.price_half_cents / price_full_cents), o que o
-- add_ons genérico (1 preço só) não suporta — por isso um catálogo à parte.
--
-- private.create_order ganha um branch para product_type = 'pizza_flavor':
-- preço = preço do sabor no tamanho escolhido, ou média dos dois preços
-- quando há um segundo sabor (meio a meio); ignora estoque/promoção (não
-- modelados para pizza nesta primeira versão). Produtos 'standard' seguem
-- exatamente o fluxo anterior (stock_quantity + effective_price_cents).

create type public.pizza_addon_application as enum ('whole', 'flavor1', 'flavor2');
...
alter table public.products
  ...
  check (product_type in ('standard', 'pizza_flavor'));
...
create table public.pizza_sizes (
  ...
  constraint pizza_sizes_name_unique unique (name),
  constraint pizza_sizes_diameter_positive check (diameter_cm > 0)
);
...
create table public.pizza_flavor_prices (
  ...
  constraint pizza_flavor_prices_price_nonneg check (price_cents >= 0)
);

create trigger pizza_flavor_prices_set_updated_at
  before update on public.pizza_flavor_prices
...
create table public.pizza_addons (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint pizza_addons_price_half_nonneg check (price_half_cents >= 0),
  constraint pizza_addons_price_full_nonneg check (price_full_cents >= 0)
);

create trigger pizza_addons_set_updated_at
  before update on public.pizza_addons
  for each row execute function public.set_updated_at();

alter table public.cart_items
  add column if not exists pizza_size_id uuid references public.pizza_sizes (id),
  add column if not exists secondary_product_id uuid references public.products (id);
...
create table public.cart_item_pizza_addons (
  cart_item_id uuid not null references public.cart_items (id) on delete cascade,
  pizza_addon_id uuid not null references public.pizza_addons (id),
  applies_to public.pizza_addon_application not null,
  created_at timestamptz not null default now(),
  primary key (cart_item_id, pizza_addon_id, applies_to)
);

create table public.order_item_pizza_addons (
  id uuid primary key default gen_random_uuid(),
  order_item_id uuid not null references public.order_items (id) on delete cascade,
  pizza_addon_id uuid references public.pizza_addons (id),
  pizza_addon_name text not null,
  applies_to public.pizza_addon_application not null,
  unit_price_cents integer not null,
  line_total_cents integer not null,
  created_at timestamptz not null default now(),
  constraint order_item_pizza_addons_unit_price_nonneg check (unit_price_cents >= 0),
  constraint order_item_pizza_addons_line_total_nonneg check (line_total_cents >= 0)
);

create index pizza_flavor_prices_size_idx on public.pizza_flavor_prices (size_id);
create index cart_items_pizza_size_idx on public.cart_items (pizza_size_id);
create index order_items_pizza_size_idx on public.order_items (pizza_size_id);
create index order_item_pizza_addons_order_item_idx
  on public.order_item_pizza_addons (order_item_id);

-- Só G ativo no lançamento; P e M ficam prontos para serem ligados depois.
insert into public.pizza_sizes (name, diameter_cm, sort_order, is_active) values
  ('P', 25, 0, false),
  ('M', 30, 1, false),
  ('G', 35, 2, true);
...
alter table public.cart_item_pizza_addons enable row level security;
alter table public.order_item_pizza_addons enable row level security;

create policy pizza_sizes_public_read
  on public.pizza_sizes for select
  to anon, authenticated
  using (private.is_admin() or is_active = true);
...
create policy cart_item_pizza_addons_insert_own
  on public.cart_item_pizza_addons for insert
  to authenticated
  with check (
    exists (
      select 1
...
-- ---------------------------------------------------------------------------
-- private.create_order — adiciona suporte a pizza (tamanho + até 2 sabores +
-- adicionais por metade), preservando o fluxo existente para product_type
-- 'standard' (estoque + effective_price_cents).
-- ---------------------------------------------------------------------------
