...
import {
  deletePromoModalBanner,
  updatePromoModalBanner,
} from '@/modules/admin/promo-modal-banners/crud';
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const result = await updatePromoModalBanner(bannerId, parsed.data);
  if (!result.ok) {
    return NextResponse.json(
      { error: result.error },
...
export async function DELETE(_request: Request, context: RouteContext) {
  const { bannerId } = await context.params;
  const result = await deletePromoModalBanner(bannerId);
  if (!result.ok) {
    return NextResponse.json(
      { error: result.error },
      { status: httpStatusFor(result.error.code) },
