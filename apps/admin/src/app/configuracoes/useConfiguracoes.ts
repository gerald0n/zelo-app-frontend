...
export function useConfiguracoes() {
  ...
  const storeForm = useForm<StoreFormValues>({
    ...
    defaultValues: {
      ...
      paymentFeeEstimatePercent: 0.99,
  ...
  useEffect(() => {
    ...
    storeForm.reset({
      ...
      paymentFeeEstimatePercent: store.paymentFeeEstimateBps / 100,
    ...
    mutationFn: (values: StoreFormValues) =>
      apiJson('/api/v1/admin/store', {
        ...
        body: JSON.stringify({
          ...
          paymentFeeEstimateBps: Math.round(
            values.paymentFeeEstimatePercent * 100,
          ),
